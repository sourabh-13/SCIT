    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title"> System Management - {{ $labels['daily_record']['label'] }} </h4>
            </div>
                        ...
                        <div class="form-group col-md-12 col-sm-12 col-xs-12 p-0 add-rcrd">
                            ...
                            <div class="col-md-1 col-sm-1 col-xs-12 p-0 r-p-15">
                                <div class="col-md-1 col-sm-1 col-xs-1 p-0">
                                    ...
                                    <button class="btn group-ico save-record-btn" type="submit">
...
<script type="text/javascript">
    $(document).on('click','.record_del_btn',function(){
        ...
        if(record_id != ''){
            ...
            $.ajax({
                type    : 'post',
                url     : "{{ url('/system/del-daily_records') }}",
                dataType: "json",
                data    : {'record_id':record_id,'_token':record_token},
                cache   : false,
...
$(document).ready(function(){
    ...
    //save a new record
    $(document).on('click','.save-record-btn',function(){
        ...
        $.ajax({
            type : 'post',
            url  : "{{ url('/system/daily-records/add') }}",
            data : { 'record_description' : record_description, 'record_score' : record_score, '_token' : record_token},
            success:function(resp){
                if(isAuthenticated(resp) == false){
                    return false;
                }
...
$(document).ready(function(){
    ...
    $(document).on('click','.delete_record_btn',function(){
        ...
        $.ajax({
            type : 'post',
            url  : "{{ url('/system/daily-records/delete/') }}"+'/'+daily_record_id,
            data : { 'daily_record_id' : daily_record_id, '_token' : record_token },
...
$(document).ready(function(){
    ...
    $(document).on('click','.status_record_change_btn',function(){
        ...
        $.ajax({
            type : 'post',
            url  : "{{ url('/system/daily-records/status/') }}"+'/'+daily_record_id,
            data : { 'daily_record_id' : daily_record_id, '_token' : record_token },
            success:function(resp){
                if(isAuthenticated(resp) == false){
                    return false;
...
<script>
    $(document).ready(function(){
        ...
        $(document).on('click','.submit-edit-daily-record',function() {
            ...
            $.ajax({
                type : 'get',
                url  : "{{ url('/system/daily-records/edit') }}",
                data : formdata,
                success:function(resp){
                    if(isAuthenticated(resp) == false){
                        return false;
